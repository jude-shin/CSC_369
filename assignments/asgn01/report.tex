	...
	\item Consider a Map/Reduce solution to a program that reads a file full with 
		...
		\begin{enumerate}
			\item What is the input/output type of the mapper and reducer?
				...
				key is the line number with type \lstinline{IntWritable}, and the value 
				are the characters that are on the line with type \lstinline{Text}.
				The output of the mapper would be a key-value pair, where the key is 
				null of type \lstinline{NullWritable}, and the value would be the 
