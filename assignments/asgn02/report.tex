...
\end{lstlisting}
			...
			\item Can you create a combiner for the program? What will be the code 
				...
				\textit{Yes, a combiner can be used for this program. I wrote my mapper 
				and reducer for compatibility with a combiner. It also just so happens 
				that I can use the code in MyReducer as the combiner. Since the reducer 
				has the same inputs and outputs as the output for the mapper, and since 
				we are partially aggregating the data, we can safely reuse the same 
				code. No additional changes need to be made to my original mapper or
				reducer code.}
